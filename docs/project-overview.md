...
랭킹 API는 대부분 KIS 순위 API를 직접 호출해 프론트 응답으로 정규화합니다.

인기 검색 종목은 별도 흐름을 사용합니다.

```text
KIS HTS조회상위20종목
  -> 종목코드 목록 획득
  -> DB public.stock에서 종목명 매핑
  -> 각 종목 현재가 API 호출
  -> 등락률과 방향 보강
  -> 프론트 응답
```

종목명은 KIS 현재가 응답에 의존하지 않고 DB `stock.short_code`, `stock.name` 매핑을 사용합니다.
